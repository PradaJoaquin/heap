...
static void prueba_destruir_datos_heap(){
    printf("\nPRUEBAS DESTRUIR DATOS HEAP\n");
    int n[] = {0, 4, 1, 2, 5, 3};
    int** n2 = malloc(sizeof(int*) * 6);
    size_t largo = 6;
    for(int i = 0; i < largo; i++){
    ...
    print_test("Prueba heap crear heap arreglo con datos a liberar", heap);
    print_test("Prueba heap no esta vacio", !heap_esta_vacio(heap));
    print_test("Prueba heap la cantidad de elementos es la correcta", heap_cantidad(heap) == largo);
    print_test("Prueba ver maximo es el correcto", *(int*)heap_ver_max(heap) == 5);
    ...
    free(n2);
    print_test("Se destruyo el heap con todos sus datos correctamente", true);
}
